...
export const Dashboard = ({ user, infoUser ,getUsers,allUsers}) => {
  ...
  const estilos = {
    container:`  flex bg-gradient-to-r from-${admin?"yellow":"blue"}-500   to-${admin?"yellow-800":"sky-800"}`,
    container2:`sticky top-0 w-1/5 h-screen bg-gradient-to-r from-${admin?"yellow-700":"sky-600"}   to-${admin?"yellow-800":"sky-400"}`,
    panelnombre:`sticky top-0 border-b-4 bg-gradient-to-r from-${admin?"yellow-600":"sky-700"}   to-${admin?"yellow-700":"sky-500"} p-3 text-gray-900 text-right    p-5 text-2xl `,
    panelcontenido: ` py-10 text-gray-800 shadow-blue-500 w-max   font-semibold ease-in duration-300 text-center rounded-3xl `,
    panelsuperior:`flex-col w-screen  bg-gradient-to-r from-${admin?"yellow-300":"sky-300"} via-neutral-500 via-${admin?"neutral":"sky"}-500 to-${admin?"neutral":"sky"}-300  bg-gradient-to-tr `,
    buttonlight:
      `text-${admin?"gray-700":"white"} bg-gradient-to-r from-${admin?"yellow":"blue"}-500  to-${admin?"yellow":"sky"}-400 hover:bg-gradient-to-br  font-medium rounded-lg text-sm px-3 py-3 text-center mr-2 mb-2"`,
      panel:`w-max flex py-10 m-2 h-max bg-gradient-to-r from-${admin?"yellow":"cyan"}-400 via-${admin?"yellow":"sky"}-500 to-${admin?"yellow":"sky"}-600  rounded-r-full  `,
    buttonred:
      `text-${admin?"gray-700":"white"} bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 py-3 my-3`,
    mensaje:
      ` text-3xl tex-center p-3 text-white  shadow-${admin?"yellow":"cyan"}-500 shadow-md w-max    font-semibold ease-in duration-300 text-center rounded-3xl `,
    titulo:
    ...
    li: `p-3 text-${admin?"gray-700":"white"}  shadow-${admin?"yellow-700":"blue-500"} shadow-md w-max  hover:text-gray-700  font-semibold ease-in duration-300 text-center rounded-3xl cursor-pointer`,
    ...
    li3: `text-lg p-4 text-${admin?"yellow":"cyan"}-800  shadow-2xl w-max shadow-${admin?"yellow":"cyan"}-900 hover:text-yellow-00  font-bold ease-in duration-300 text-center rounded-3xl cursor-pointer`,
    lisesion:
      ` p-3 my-5 text-${admin?"yellow":"white"} shadow-${admin?"yellow":"cyan"}-800 shadow-xl w-max  hover:text-${admin?"yellow":"cyan"}-800  font-semibold ease-in duration-300 text-center rounded-3xl cursor-pointer`,
    label: ``,
    input: `p-2 rounded `,
  ...
  const opcion = (e) => {
    switch (e) {
      case "":
        return <BienvenidoPanel administrador={admin} estilos={estilos}></BienvenidoPanel>;
      case "FormUpdate":
        return <FormUpdateUser user={user} estilos={estilos}></FormUpdateUser>;
      case "estadisticas":
        return <h1 className={estilos.li}>estadisticas</h1>
      case "publicaciones":
        return (
          user &&
          user?.cars
