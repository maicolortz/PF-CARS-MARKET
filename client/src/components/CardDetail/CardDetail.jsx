...
function CardDetail() {
  ...
  const { name, image, brand, model, year, color, oil, gate, kilometres, descriptionLong, price, condition, transmition } = carsDetail;
  const details = [
    ...
    <div>
      {carsDetail && (
        <div className="2xl:container 2xl:mx-auto lg:py-8 lg:px-20 md:py-12 md:px-6 py-9 px-4 ">
          ...
          <Link to={'/home'}>
            <button title="Regresar al inicio" className=" text-white bg-blue-700 hover:bg-blue-800 focus:outline-none shadow-md shadow-black rounded-full text-center mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-900 dark:focus:ring-blue-800 font-semibold text-lg leading-4  w-auto py-4 px-4 lg:mt-12 mt-6">Regresar</button>
          </Link>
          ...
          <div className="flex justify-center items-center lg:flex-row flex-col gap-0 mt-6">
            ...
            <div className="  w-full sm:w-96 md:w-8/12 lg:w-6/12 border-4 rounded-2xl p-6 pb-12 items-center">
              <div class="flex justify-between items-center">
                <p className=" focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 font-medium text-lg leading-4 text-gray-600 ">{carsDetail.brand} / {carsDetail.model} / {carsDetail.color} / {carsDetail.year}</p>
                <a href="#top" title="Añadir a Favoritos" class="md:text-6xl text-gray-300  hover:text-red-500 duration-300">&hearts;</a>
              </div>
              <h2 className="font-semibold lg:text-4xl text-3xl lg:leading-9 leading-7 text-gray-800 mt-4">{name}</h2>
              <p className=" font-normal pb-7 text-base leading-6 text-gray-600 mt-7">{descriptionLong}</p>
              <div>
                {
                  details.map((el) => {
                    return (
                      <div className="py-4 border-b border-t border-gray-200 flex items-center justify-between">
                        <p className="text-lg leading-4 font-semibold text-gray-800">{el.name}</p>
                        <div className="flex items-center justify-center">
                          <p className="text-lg leading-none text-gray-600">{el.detalle} {el.name === "Kilometraje: " && "Km" }</p>
                        </div>
                      </div>
                    )
                  })
